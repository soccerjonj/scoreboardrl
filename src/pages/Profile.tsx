  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        // Try to load bio + favorite_car + show_on_leaderboard (may not exist if migration hasn't run)
        try {
          const { data: extData } = await supabase
            .from("profiles")
            .select("bio, favorite_car, show_on_leaderboard")
            .eq("user_id", user.id)
            .single();
          ...
          setShowOnLeaderboard(extData?.show_on_leaderboard ?? true);
        } catch {
